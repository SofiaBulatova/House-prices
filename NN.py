...
class MyNN(nn.Module):
    """
        ...
        -> выходной слой на 1 нейрон (для регрессии).
...
class NeuralNet(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, hidden_dims, dropout, lr, epochs, batch_size,
                 random_state, patience, val_fraction, step_size, gamma):
    ...
    def predict(self, X):
        ...
        self.model_.eval()
        with torch.no_grad():
            preds = self.model_(X_tensor).numpy()
        return preds